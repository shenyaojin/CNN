...
for iter in range(len(img_list)):
    if img_list[iter].imgsize[0] >= 60:
        ...
        count += 1
# Select the images without glass
tmp_imglist = []
count = 0
for iter in range(len(cleaned_imglist)):
    if cleaned_imglist[iter].sunglass == 1:
        tmp_imglist.append(cleaned_imglist[iter])
        count += 1
...
T1 = time.time()
history = model.fit(X_train, Y_train, batch_size = 100, epochs = 10, verbose = 1, shuffle = True)
T2 = time.time()
...
model.save('model_with_glass.h5')
print("                               DONE                              ")
